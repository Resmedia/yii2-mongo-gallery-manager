...
class GalleryBehavior extends Behavior
{
     ...
     */
    public $tableName = 'image_gallery';
    ...
    public function getImages()
    {
        if ($this->_images === null) {
            ...
            $imagesData = $query
                ->select(['_id', 'name', 'description', 'rank'])
                ->from($this->tableName)
    ...
    {
        // This need if image does not exist
    ...
    public function getGalleryId()
    {
        ...
        if (is_array($pk)) {
            return implode($this->pkGlue, $pk['oid']);
        } else {
            $id = (string)$pk;
            return $id;
        }
    ...
    public function addImage($fileName)
    {
        ...
        $collection->update(['_id' => $id], ['rank' => $id]);
    ...
    public function arrange($order)
    {
        ...
        foreach ($order as $k => $v) {
            ...
            $collection = \Yii::$app->mongodb->getCollection($this->tableName);
            /** @var $collection \yii\mongodb\Collection */
            $collection->update(['_id' => $k], ['rank' => $orders[$i]]);
            $i++;
        }
